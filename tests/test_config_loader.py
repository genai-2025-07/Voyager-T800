...
def test_config_loader_init_and_load_base(tmp_path, create_yaml_file, temp_config_dir):
    """Test that the ConfigLoader loads the base config correctly."""
    default_content = {
        "app": {"name": "test_app_base", "env": "dev"},
        "model": {"openai": {"model_name": "base_model"}},
    }
    ...
    assert settings.app.name == "test_app_base"
    assert settings.app.env == "dev"
    assert settings.model.openai.model_name == "base_model"


def test_config_loader_override_with_file(tmp_path, create_yaml_file, temp_config_dir):
    """Test that an override file merges correctly."""
    default_content = {
        "app": {"name": "base_app", "version": "0.1.0"},
        "embedding": {"provider": "default_provider"},
    }
    override_content = {
        "app": {"version": "1.0.0"},
    ...
    assert settings.app.name == "base_app"
...
def test_config_loader_env_override_path(tmp_path, create_yaml_file, temp_config_dir, set_env):
    """Test that VOYAGER_CONFIG environment variable specifies override path."""
    default_content = {"app": {"name": "base_app"}}
...
def test_config_loader_app_env_override(tmp_path, create_yaml_file, temp_config_dir, set_env):
...
def test_config_loader_env_var_expansion(tmp_path, create_yaml_file, temp_config_dir, set_env):
    """Test that environment variables are expanded correctly."""
    default_content = {
        "app": {"name": "${APP_NAME}", "version": "${APP_VERSION:1.0.0}"},
...
def test_config_loader_validation_failure(tmp_path, create_yaml_file, temp_config_dir):
    """Test that invalid config values raise Pydantic ValidationError."""
    default_content = {
        "app": {"env": "invalid_env_type"}, # AppSettings.env is Literal["dev", "prod", "test"]
    }
    create_yaml_file(temp_config_dir, "default.yaml", default_content)
...
def test_config_loader_empty_config_file(tmp_path, create_yaml_file, temp_config_dir):
    """Test loading an empty YAML file."""
    create_yaml_file(temp_config_dir, "default.yaml", {})

    loader = ConfigLoader(project_root=tmp_path)
    settings = loader.get_settings()
    # Ensure defaults are applied for all nested models
    assert settings.app.name == "voyager-t800"
...
def test_config_loader_get_settings_not_initialized():
