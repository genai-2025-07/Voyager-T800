...
def read_file_content(path: Path) -> str:
    ...
    for encoding in ('utf-8', 'latin-1'):
        try:
            ...
            logger.warning(f'Could not read {path} with {encoding}: {e}')
    ...
    if suffix == '.json':
        try:
            ...
            logger.error(f'UnicodeDecodeError while parsing JSON {path}: {ude}')
        except json.JSONDecodeError as je:
            logger.error(f'JSONDecodeError while parsing {path}: {je}')
        except Exception as e:
            logger.error(f'Unexpected error parsing JSON {path}: {e}')
